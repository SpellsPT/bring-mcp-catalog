...
jest.mock('bring-shopping', () => {
  ...
  return jest.fn().mockImplementation(() => ({
    login: mockLogin,
    // Removals first check the name is really on the list, so the list holds
    // exactly the awkward names these tests remove.
    getItems: jest.fn().mockResolvedValue({
      uuid: '11111111-2222-4333-8444-555555555555',
      status: 'SHARED',
      purchase: [
        { name: 'Fish & Chips', specification: '' },
        { name: 'Salt + Pepper', specification: '' },
      ],
      recently: [],
    }),
