...
export function registerUserTools(server: McpServer, bc: BringService) {
  ...
  registerTool({
    ...
    actionFn: async (_args, bc) => {
      ...
      const lists = loadListsOutputSchema.parse(await bc.loadLists()).lists;
      ...
      // or deleted) would send every later tool call at a dead uuid, so it is
      // only honoured while that list still exists; otherwise fall through.
      if (defaultListSetting && lists.some((list) => list.listUuid === defaultListSetting.value)) {
      ...
      if (lists.length === 1 && lists[0]?.listUuid) {
        return getDefaultListOutputSchema.parse({
      ...
      // Several lists and no usable default is genuinely ambiguous: name the
      // candidates so the assistant can ask, rather than just failing.
      const candidates = lists.map((list) => `"${list.name}" (${list.listUuid})`).join(', ');
      return getDefaultListOutputSchema.parse({
        listUuid: null,
        source: 'not-configured',
        message:
          lists.length === 0
            ? 'No default list is configured and the account has no lists.'
            : `No default list is configured and the account has ${lists.length} lists. ` +
              `Ask which one, then pass its listUuid: ${candidates}`,
      });
    },
